 ...
 *
 */
package org.cambi.oauth.twitter;
...
public class TwitterAuthenticator {
	...
	private static final Logger out = LoggerFactory.getLogger(TwitterAuthenticator.class);
	 ...
	 *
	 * @param out            Where to print the progress information and messages to
	 *                       the user
	...
	/**
	 * Lazily initialize an HTTP request factory which embeds the OAuth tokens
	 * required by the Twitter APIs
	 *
	...
	/**
	 * Create a new authenticated HTTP request factory which embeds the OAuth tokens
	 * required by the Twitter APIs
	 *
	 * @return The authenticated HTTP request factory
	 */
	private HttpRequestFactory createRequestFactory() throws TwitterAuthenticationException, IOException {
		out.info("... starting authorization ...");

		OAuthHmacSigner signer = new OAuthHmacSigner();
		signer.clientSharedSecret = consumerSecret;

		out.info("... getting temporaty token ...");
		...
		out.info("... retreiving pin ...");

		String providedPin = retrievePin(authorizeUrl);

		out.info("... retreiving access token ...");
		...
		out.info("... i am done with authorization, creating request factory");
	...
	private OAuthCredentialsResponse getTemporaryToken(final OAuthHmacSigner signer)
		...
		out.info("Aquired temporary token...\n");
	 ...
	 *
	 * @param authorizeUrl The URL embedding the temporary tokens to be used to
	 *                     request the PIN
	 * @return The PIN as it is entered by the user after following the Twitter
	 *         OAuth wizard
	 */
	private String retrievePin(final OAuthAuthorizeTemporaryTokenUrl authorizeUrl)
		...
		try {
			out.info("Go to the following link in your browser:\n" + authorizeUrl.build());
	...
	/**
	 * Exchange the temporary token and the PIN for an access token that can be used
	 * to invoke Twitter APIs
	 *
	 * @param providedPin The PIN that the user obtained when following the Twitter
	 *                    OAuth wizard
	 * @param signer      The HMAC signer used to cryptographically sign requests to
	 *                    Twitter
	 ...
	 */
	private OAuthCredentialsResponse retrieveAccessTokens(final String providedPin, final OAuthHmacSigner signer,
